...
# ----------------------------------------------------
# 9. 네이버 쇼핑 카테고리 매핑 (NEW - 사용자 요청 반영)
# ----------------------------------------------------
# 무신사 예측 결과(품목/성별)에 따른 네이버 쇼핑의 세부 카테고리 코드 (예시)
NAVER_CATEGORY_MAPPING = {
    # 품목: (남성 코드, 여성 코드)
    "top": ("50000003", "50000004"),      # 남성 상의, 여성 상의
    "bottom": ("50000005", "50000006"),   # 남성 하의, 여성 하의
    "outer": ("50000007", "50000008")     # 남성 아우터, 여성 아우터
}
...
# ----------------------------------------------------
# 8. 네이버 쇼핑 검색 링크 생성 함수 (최종 수정: 카테고리 ID 적용)
# ----------------------------------------------------
def generate_naver_shopping_link(search_keywords, item_type, gender):
    """
    최적화된 검색 키워드와 ML 예측 결과를 사용하여 네이버 쇼핑 가격 비교 URL을 생성합니다.
    item_type (top/bottom/outer)과 gender를 기반으로 cat_id를 삽입합니다.
    """
    ...
    # 💡 카테고리 코드 결정 로직
    naver_category_tuple = NAVER_CATEGORY_MAPPING.get(item_type)
    
    cat_id = "" # 기본값은 빈 문자열
    if naver_category_tuple:
        if gender.lower() == 'male':
            cat_id = naver_category_tuple[0] # 남성 코드
        elif gender.lower() == 'female':
            cat_id = naver_category_tuple[1] # 여성 코드
        # 성별이 'male' 또는 'female'이 아니면 cat_id는 빈 문자열 유지
        
    # 네이버 쇼핑 URL 형식: BASE_URL?query=...&cat_id=...&frm=NVSHATC
    full_url = f"{NAVER_SHOPPING_BASE_URL}?query={encoded_query}&cat_id={cat_id}&frm=NVSHATC"
    
    return full_url, query, cat_id
...
if run_button or st.session_state['analysis_done']:
    ...
    if st.session_state['analysis_done']:
        ...
        # ----------------------------------------------------
        # D. 네이버 쇼핑 가격 비교 (Gemini 키워드 최적화 및 카테고리 적용)
        # ----------------------------------------------------
        st.markdown("---")
        ...
        # 사용자 입력창: Session State를 사용하여 값 유지
        naver_input = st.text_input(
            "📝 제품명 입력:",
            value=st.session_state['naver_shopping_input_value'], 
        ...
        if naver_button:
            if not st.session_state['naver_shopping_input_value']:
                ...
                # 필요한 변수들을 세션 상태에서 로드
                item_type = st.session_state['item_type_select']
                gender = st.session_state['extracted_json']['gender']
                ...
                # 8. 네이버 쇼핑 링크 생성 함수 호출 (수정된 함수)
                naver_url, naver_query, cat_id = generate_naver_shopping_link(optimized_query, item_type, gender)
                
                st.markdown(f"**🔍 최적화된 검색 키워드:** `{naver_query}`")
                
                # 새로 적용된 카테고리 ID 정보 출력
                st.markdown(f"**🛒 적용된 카테고리 ID (cat_id):** `{cat_id if cat_id else 'N/A'}` ({gender.upper()} {item_type.upper()} 기반)")
